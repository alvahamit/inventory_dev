...
<h1>Hi!! you can update purchases here.</h1><hr>
                    ...
                    <div class="col-md-6">
                        <div class="form-label-group">
                            <input type="date"
                                   ...
                                   value="{{old('receive_date', $purchase->receive_date)}}">
                        ...
                        <div class="form-group">
                            <select name="user_id"
                                    ...
                                    @foreach($data as $item)
                                        @if($purchase->user_id == $item->id OR old('user_id') == $item->id)
                        ...
                        <div class="form-group">
                            <select name="purchase_type"
                                ...
                                @for($i=0; $i < count($purchase_types); $i++)
                                    @if($purchase_types[$i] === $purchase->purchase_type OR old('purchase_type') === $purchase_types[$i])
                                    <option value="{{$purchase_types[$i]}}" selected="selected">{{$purchase_types[$i]}}</option>
                                    @else
                                    <option value="{{$purchase_types[$i]}}">{{$purchase_types[$i]}}</option>
                                    @endif
            ...
            <table class="table" id="items">
                ...
                <tbody>
                    @foreach($purchase->products->all() as $item)
                        @for($i = 1; $i <= count($purchase->products()); $i++)
                        <tr id="set{{$i}}" class="set">
                            <td>
                            ...
                            <td><input type="number" min="0" id="qty{{$i}}" name="quantities[]" class="form-control qty"/></td>
                            <td><input type="number" min="0" id="unit_price{{$i}}" name="unit_prices[]" class="form-control unit_price"/></td>
                            ...
                            <td><input type="number" min="0" id="item_total{{$i}}"  name="item_totals[]" class="form-control item_total"/></td>
                            <td><button id="remove{{$i}}" type="button" class="close" data-dismiss="alert">&times;</button></td>
                        </tr>
                        @endfor
                    @endforeach
